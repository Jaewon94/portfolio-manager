...
import { Note, Project } from '@/types/api';
...
export default function NotesPage() {
  ...
  const [activeTab, setActiveTab] = useState<'learn' | 'change' | 'research'>('learn');
  ...
  const fetchNotes = async () => {
    try {
      ...
      if (selectedProject) {
        // 특정 프로젝트의 노트 조회
        const response = await noteService.getNotesByType(selectedProject.toString(), activeTab);
  ...
  const getTabIcon = (type: string) => {
    switch (type) {
      ...
      case 'change': return <GitBranch className="w-4 h-4" />;
  ...
  const getTabDescription = (type: string) => {
    switch (type) {
      case 'learn': return '새로운 기술과 개념을 학습한 내용을 기록하세요';
      case 'change': return '프로젝트의 변경사항과 업데이트를 추적하세요';
      case 'research': return '기술 조사와 리서치 결과를 정리하세요';
      default: return '';
    }
  };
                    ...
                    {loading || projectsLoading ? (
                      <div className="flex items-center justify-center h-64">
                        <div className="text-gray-500">
                          {projectsLoading ? '프로젝트를 불러오는 중...' : '노트를 불러오는 중...'}
                        </div>
                      </div>
                    ) : !selectedProject ? (
                      ...
                      <div className="flex flex-col items-center justify-center h-64">
                        <div className="text-gray-500 mb-4">
                          {searchQuery ? '검색 결과가 없습니다' : `${getTabLabel(tabType)} 노트가 없습니다`}
                        </div>
                            ...
                            <CardContent>
                              <div className="space-y-3">
                                ...
                                <p className="text-sm text-gray-600 line-clamp-3">
                                  {typeof note.content === 'object' && note.content.text
                                    ? note.content.text
                                    : '내용 없음'}
                                </p>
