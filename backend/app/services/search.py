...
class SearchService:
    ...
    # 상수 정의
    VALID_CONTENT_TYPES = ["project", "note", "user"]
    VALID_AUTOCOMPLETE_TYPES = ["all", "project", "note", "tag"]
    DEFAULT_LIMIT = 20
    DEFAULT_OFFSET = 0
    MAX_LIMIT = 100
    ...
    async def search_all(
        ...
        if "note" in content_types:
            note_results = await self._search_notes(query, user_id, limit, offset)
            results["notes"] = note_results["notes"]
            results["total_count"] += note_results["count"]

        # 사용자 검색
        if "user" in content_types:
            user_results = await self._search_users(query, limit, offset)
    ...
    def _validate_autocomplete_type(self, content_type: str) -> None:
        """autocomplete content_type 검증"""
        if content_type not in self.VALID_AUTOCOMPLETE_TYPES:
            raise ValueError(
                f"Invalid autocomplete type: '{content_type}'. "
                f"Must be one of: {self.VALID_AUTOCOMPLETE_TYPES}"
            )
    ...
    async def _search_projects(
        ...
        return {"projects": projects, "count": total_count}
    ...
    async def _search_notes(
        ...
        stmt = (
            select(Note)
            .join(Project)
            .options(selectinload(Note.project).selectinload(Project.owner))
        )
        ...
        return {"notes": notes, "count": total_count}
    ...
    async def get_autocomplete_suggestions(
        ...
        """
        # content_type 검증
        self._validate_autocomplete_type(content_type)

        suggestions = []

        if content_type in ["all", "project"]:
            # 프로젝트 제목 자동완성
            project_suggestions = await self._get_project_title_suggestions(
                query, limit
            )
    ...
    async def _get_tag_suggestions(self, query: str, limit: int) -> List[str]:
        """태그 자동완성"""
        # 프로젝트 태그
        project_tags_stmt = select(func.unnest(Project.tags).label("tag")).where(
            and_(
                Project.visibility == ProjectVisibility.PUBLIC,
                Project.status == ProjectStatus.ACTIVE,
            )
        ...
        # 서브쿼리로 감싸서 필터링 및 집계
        final_stmt = (
            select(union_stmt.c.tag, func.count("*").label("count"))
            .where(union_stmt.c.tag.ilike(f"%{query}%"))
            .group_by(union_stmt.c.tag)
    ...
    async def get_popular_searches(self, limit: int = 10) -> List[Dict[str, Any]]:
        """인기 검색어 조회 (태그 기반)"""
        # 프로젝트와 노트의 모든 태그 집계
        project_tags = select(func.unnest(Project.tags).label("tag")).where(
            and_(
                Project.visibility == ProjectVisibility.PUBLIC,
                Project.status == ProjectStatus.ACTIVE,
            )
        ...
        return [{"keyword": row[0], "count": row[1]} for row in result.fetchall()]
    ...
    async def get_search_stats(self) -> Dict[str, Any]:
        ...
        user_count_stmt = select(func.count(User.id)).where(User.is_verified == True)
