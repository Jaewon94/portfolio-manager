...
class AuthService:
    ...
    async def get_oauth_user_info(self, provider: str, oauth_code: str) -> OAuthUserInfo:
        """
    ...
    async def _get_google_user_info(self, oauth_code: str) -> OAuthUserInfo:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if "names" in user_data and user_data["names"]:
                primary_name = next((n for n in user_data["names"] if n.get("metadata", {}).get("primary")), user_data["names"][0])
            ...
            if "emailAddresses" in user_data and user_data["emailAddresses"]:
                primary_email = next((e for e in user_data["emailAddresses"] if e.get("metadata", {}).get("primary")), user_data["emailAddresses"][0])
                email = primary_email.get("value")
            
            # 프로필 사진 추출
            avatar_url = None
            if "photos" in user_data and user_data["photos"]:
                primary_photo = next((p for p in user_data["photos"] if p.get("metadata", {}).get("primary")), user_data["photos"][0])
            ...
            user_id = resource_name.replace("people/", "") if resource_name else email.split("@")[0]
    ...
    async def _get_kakao_user_info(self, oauth_code: str) -> OAuthUserInfo:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if not name:
                name = email.split("@")[0] if "@" in email else f"kakao_{user_data['id']}"
    ...
    async def create_or_update_oauth_user(self, provider: str, user_info: OAuthUserInfo) -> User:
        """
