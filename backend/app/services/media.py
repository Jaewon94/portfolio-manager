...
class MediaService:
    ...
    ALLOWED_ARCHIVE_TYPES = {
        ...
        "application/gzip",
        "application/vnd.rar",
        "application/x-rar-compressed",  # RAR 지원 추가
    ...
    async def get_media_by_id(
        self, media_id: int, user_id: Optional[int] = None
    ) -> Optional[Media]:
        """미디어 ID로 조회"""
    ...
    async def get_media_by_target(
        ...
        stmt = select(Media).where(
            and_(Media.target_type == target_type, Media.target_id == target_id)
        )
        ...
        if user_id:
            ...
            for media in media_list:
                if await self._check_media_access_permission(media, user_id):
    ...
    async def update_media(
        ...
        # 대상 소유권 확인
        if not await self._check_target_ownership(
            media.target_type, media.target_id, user_id
        ):
            raise HTTPException(
    ...
    async def bulk_delete_media(
        self, media_ids: List[int], user_id: int
    ) -> Dict[str, Any]:
        """미디어 일괄 삭제"""
        deleted_count = 0
        failed_ids = []
    ...
    async def get_storage_stats(self, user_id: Optional[int] = None) -> Dict[str, Any]:
        ...
        type_stats_stmt = select(
            Media.type, func.count(Media.id).label("count")
        ).group_by(Media.type)

        visibility_stats_stmt = select(
            Media.is_public, func.count(Media.id).label("count")
    ...
    async def _check_upload_permission(
        self, target_type: MediaTargetType, target_id: int, user_id: int
    ) -> None:
        """업로드 권한 확인"""
    ...
    async def _check_media_access_permission(
        self, media: Media, user_id: Optional[int]
    ) -> bool:
        """미디어 접근 권한 확인"""
        ...
        # 소유권 확인
        return await self._check_target_ownership(
            media.target_type, media.target_id, user_id
        )

    async def _build_access_filter(self, user_id: int, only_public: bool):
        """접근 권한 필터 생성"""
        if only_public:
    ...
    async def _process_image_buffer(
        self, image_buffer, filename: str
